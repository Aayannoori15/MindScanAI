...
Reads dataset/mental_health_multimodal.csv (4000 rows, 18 features +
Mental_Health_Status + D/A/S scores), z-scores features with the same
(mean, std) table the API uses at inference (backend.core.dataset_spec.
FEATURE_STATS, computed from this same file), and writes:
...
CSV_PATH = ROOT / "dataset" / "mental_health_multimodal.csv"
...
def train_classifier(x_train, y_train, x_val, y_val, epochs=200, lr=1e-3):
    ...
    for epoch in range(epochs):
        ...
        model.eval()
        with torch.no_grad():
            val_pred = model(xv).argmax(1)
            acc = (val_pred == yv).float().mean().item()
        if acc > best_acc:
            best_acc = acc
            best_state = {k: v.clone() for k, v in model.state_dict().items()}
        if (epoch + 1) % 20 == 0:
            print(f"  [classifier] epoch {epoch+1}/{epochs} loss={loss.item():.4f} val_acc={acc:.4f}")

    model.load_state_dict(best_state)
    print(f"classifier best val_acc={best_acc:.4f}")
    return model


def train_regressor(name, x_train, y_train, x_val, y_val, score_max, epochs=300, lr=1e-3):
    ...
    for epoch in range(epochs):
        ...
        if mae < best_mae:
            ...
            best_state = {k: v.clone() for k, v in model.state_dict().items()}
        if (epoch + 1) % 30 == 0:
            print(f"  [{name}] epoch {epoch+1}/{epochs} loss={loss.item():.4f} val_mae={mae:.3f}")
...
def main():
    ...
    idx = np.arange(len(x))
    idx_train, idx_val = train_test_split(idx, test_size=0.15, random_state=42, stratify=y_status)

    x_train, x_val = x[idx_train], x[idx_val]
    y_status_train, y_status_val = y_status[idx_train], y_status[idx_val]
    ...
    for name, col in TARGETS.items():
        y_train, y_val = y_scores[name][idx_train], y_scores[name][idx_val]
